...
for (const file of ['math', 'entry']) {
  const transformResult = await server.transformRequest(`./${file}.ts`, {
    ssr: true,
  })
...
`
  writeFileSync(`./${file}.js`, code, 'utf-8')

  console.log(toVisualizer(transformResult), '\n')
}

console.log('\nRun command:')
console.log('node --inspect-brk wrapper.mjs')
